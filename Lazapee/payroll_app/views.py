...
def update_employee(request, pk):
    if request.method == "POST":
        ...
        # Check if required fields are not empty
        if not (newname and newid and newrate and newallowance):
            messages.info(request, 'All fields are required.')
            ...
            Employee.objects.filter(pk=pk).update(name=newname, id_number=newid, rate=newrate, allowance=newallowance)
...
def view_payslip(request, pk):
    ...
    if d.pay_cycle == 2:
        return render(request, "payroll_app/view_payslip2.html", {'d':d, 'id':id})
    elif d.pay_cycle == 1:
        return render(request, "payroll_app/view_payslip1.html", {'d':d, 'id':id})
